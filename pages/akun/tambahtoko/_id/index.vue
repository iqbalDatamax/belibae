      ...
      <div class="flex p-3 flex-row items-center">
          ...
          <div class="inline-flex w-full">
              <div class="w-full inline-block relative">
                <p class="text-xl text-purple-700">Buat Tokomu Gratis</p>
              </div>
          </div>
      </div>
  </header>
  <div class="pt-16 m-1">
  	<section class="mt-2 px-3">
          <form class="flex flex-col" @submit.prevent="">
            ...
            <button class="mb-8 hover:bg-purple-700 text-white font-bold py-2 rounded shadow-lg hover:shadow-xl transition duration-200" type="submit" style="background: linear-gradient(90deg, #667eea 0%, #764ba2 100%)" @click="saveToko">Submit Toko</button>
          </form>
      </section>
  </div>
</div>
...
export default {
  ...
  async asyncData({store, params}){
    await Promise.all([
      store.dispatch('alamat/getAlamatUserId', params.id)
    ])
    return
  },
  data() {
  ...
  ...mapActions('toko', [
    ...
    saveToko(){
      if(this.almt === ''){
        alert('Alamat belum dimasukan')
      }else if(this.namatoko === '' || this.deskripsi === ''){
        alert('Nama Toko dan Deskripsi tidak boleh kosong');
      }else{
        let data = {
        ...
        this.tambahTokoData(data).then(() => {
          alert('Berhasil menambah toko')
          this.$router.push('/akun')
        });
      }   
    },
    // ubahToko(){
    //   this.almt = this.tokoUser.alamats.alamat_lengkap;
    //   this.namatoko = this.tokoUser.namatoko;
    //   this.deskripsi = this.tokoUser.deskripsi;
    //   this.userId = this.tokoUser.userId;
    //   this.idtoko = this.tokoUser.id;
    //   this.foodbae = this.tokoUser.foodbae;
    //   this.status = this.tokoUser.status;
    // },
    // editToko(){
    //   let dt = {
    //     userId : this.$route.params.id,
