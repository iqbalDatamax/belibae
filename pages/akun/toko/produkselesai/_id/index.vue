  ...
  <div v-else>
      <div class="pb-4">
        ...
        <div class="flex justify-center pt-2 pb-6">
          <button v-if="size < transaksis.totalItems" @click="next" class="border border-purple-500 hover:text-white hover:bg-purple-500 rounded px-10 py-1 text-purple-500 bg-white text-center text-semibold ml-4"> Lainnya </button>
        </div>
    </div>
    <div class="w-full pt-24">
      <section class="block fixed inset-x-0 bottom-0 z-10 bg-white border-t-2 shadow py-2">
        <div class="flex justify-between">
          ...
          <p class="font-semibold text-xl text-purple-700 pr-4">{{transaksis.totalItems}}</p>
        </div>
      </section>
    </div>
</div>
...
export default {
  ...
  async asyncData({store, params}){
    var adddays=parseInt(-30) // number of days to add or subtract
...
//================================================================
    ...
    var data = `?startDate=${dts}&endDate=${dte}&page=0&size=20&status=6&toko=${params.id}`;
    await Promise.all([
  ...
  created() {
    var dts = new Date()
    var dte = new Date()
    this.startDate = this.formatDate(dts.setDate(dts.getDate() - 30));
  ...
  methods: {
    ...
    next(){
      ...
      let data = `?startDate=${this.startDate}&endDate=${this.endDate}&page=0&size=${this.size}&status=6&toko=${this.$route.params.id}`
