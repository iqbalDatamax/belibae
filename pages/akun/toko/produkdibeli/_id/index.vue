      ...
      <div class="flex p-3 flex-row items-center">
          ...
          <div class="inline-flex w-full">
              <div class="w-full inline-block relative">
                <p class="text-lg text-purple-700">List Produk dibeli - silahkah dikemas</p>
              </div>
          </div>
      </div>
  </header>
            ...
            <div class="h-44 flex mt-3 px-3 py-2 bg-white shadow-lg">
              ...
              <div class="y-2 pl-3">
                ...
                <div class="flex pt-3 items-center">
                  <p class="text-xs text-gray-700">Jika Produk tersedia, Klik konfirmasi agar kurir menjemput produk</p>
                  <button @click="konfirmasi(ut.uuid)" class="text-xs text-white hover:bg-purple-500 hover:text-white border bg-blue-500 rounded py-1 px-2 outline-none focus:shadow-outline h-8">Konfirmasi</button>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  async asyncData({store, params}){
    var adddays=parseInt(-6) // number of days to add or subtract
...
//================================================================
    ...
    var data = `?startDate=${dts}&endDate=${dte}&page=0&size=30&status=2&toko=${params.id}`;
    await Promise.all([
  ...
  created() {
    var dts = new Date()
    var dte = new Date()
    this.startDate = this.formatDate(dts.setDate(dts.getDate() - 6));
  ...
  getDataTanggal() {
    let data = `?startDate=${this.startDate}&endDate=${this.endDate}&page=0&size=30&status=2&toko=${this.$route.params.id}`
  ...
  konfirmasi(id){
    ...
    let dt = `?startDate=${this.startDate}&endDate=${this.endDate}&page=0&size=30&status=2&toko=${this.$route.params.id}`
    this.updateStatus(data).then(() => {
      this.getTransaksiData(dt).then(() => {
        this.transaksis = this.$store.state.transaksi.transaksiToko
      });
      alert('pesanan siap dijemput')
    })
  },
}, 
