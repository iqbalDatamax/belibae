        ...
        <div class="flex justify-between">
          <div class="text-left pl-4 py-2">
            <span class="text-gray-600 text-lg">
              <p class="mb-0">Komisi Penjual</p>
            </span>
          </div>
          <p class="font-semibold text-xl text-purple-700 pr-4">Rp{{komisipenjual()}}</p>
        </div>
      </section>
    </div>
</div>
...
//================================================================
    ...
    var data = `?startDate=${dts}&endDate=${dte}&page=0&size=50&status=5&toko=${params.id}`;
    await Promise.all([
  ...
  methods: {
    ...
    komisipenjual(){
  	  let total = 0;
  	  let transaksi = this.transaksis.orders
      transaksi.forEach((trans) => {
        total += trans.laba*30/100 //hitung komisi untuk penjual jika barang dikembalikan
      })
	...
	getDataTanggal() {
	    let data = `?startDate=${this.startDate}&endDate=${this.endDate}&page=0&size=50&status=5&toko=${this.$route.params.id}`
